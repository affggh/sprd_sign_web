...
bool getpubkeyfrmPEM(sprd_rsapubkey * sprdPubk, char *path_key)
{
	...
	const BIGNUM *e = RSA_get0_e(p_rsa);
  const BIGNUM *n = RSA_get0_n(p_rsa);
	int e_size = BN_num_bytes(e);
	BN_bn2bin(e, ((unsigned char *)&(sprdPubk->e) + 4 - e_size));
	while (e_size < 4) {
